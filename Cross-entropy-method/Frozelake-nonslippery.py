...
def iterate_batches(env,net,batch_size):
    ...
    sm = nn.Softmax(dim=1)
    while True:
        # Avoid slow list-of-arrays tensor construction
...
if __name__=="__main__":
    ...
    # Create a non-slippery FrozenLake env and wrap for one-hot observations
    ...
    for iter_no,batch in enumerate(iterate_batches(env,net,BATCH_SIZE)):
        ...
        loss_v.backward()
        optimizer.step()
        print("%d: loss=%.3f,reward_mean=%.1f,reward_bound=%.1f,batch=%d" %(iter_no,loss_v.item(),reward_mean,reward_bound,len(full_batch)))
